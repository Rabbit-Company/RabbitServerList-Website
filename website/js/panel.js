...
function renderMyMinecraftServers(){
	...
	for(let i = 0; i < servers.length; i++){
		...
		data += "<td class='text-center px-4 py-4 whitespace-nowrap text-sm text-gray-500'><a id='banner-" + servers[i].id + "' href='#'><img class='rounded-t-md w-full' src='https://api.rabbitserverlist.com/v1/server/minecraft/" + servers[i].id + "/banner' /></a><span class='w-full inline-flex items-center px-2.5 py-0.5 text-sm rounded-b-md font-medium " + online_color + "'><a href='#'>" + servers[i].ip + "</a></span></td>";
		...
		data += "<input id='uploadBanner-" + servers[i].id + "' type='file' accept='image/gif' class='hidden'";
		data += "</tr>";
	}
	...
	for(let i = 0; i < servers.length; i++){
		document.getElementById('banner-' + servers[i].id).addEventListener('click', () => {
			document.getElementById('uploadBanner-' + servers[i].id).click();
		});

		document.getElementById('uploadBanner-' + servers[i].id).addEventListener('input', () => {
			let fileInput = document.getElementById('uploadBanner-' + servers[i].id);
